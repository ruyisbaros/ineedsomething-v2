...
resource "aws_security_group" "autoscaling_group_sg" {
  ...
  description = "Allows internet access for launched ASG"
  vpc_id      = aws_vpc.main.id
  ...
  ingress {
    ...
    description     = "Allows SSH traffic into webserver through bastion"
  }
  ...
  ingress {
    ...
    protocol        = "TCP"
    security_groups = [aws_security_group.alb_sg.id]
    description     = "Allows access into webserver through ALB"
  }
...
resource "aws_security_group" "elasticache_sg" {
  ...
  description = "Allows access to the elasticache service"
  vpc_id      = aws_vpc.main.id
  ...
  ingress {
    ...
    protocol        = "TCP"
    security_groups = [aws_security_group.bastion_host_sg.id]
    description     = "Allows access to the Redis server through bastion host"
  }
  ...
  ingress {
    ...
    description     = "Allows access to the Redis server through ASG"
  }
